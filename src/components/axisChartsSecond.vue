<template>
<div>
  <div class="echarts w100 axis" v-for="(item) in ids" :key="item.id" :id='item.id'></div>
</div>
</template>
...
export default {
  ...
  mounted() {
    for (let i in this.ids) {
      this.drawLineA(this.ids[i].id, this.chartData[i]);
    ...
    drawLineA(id, data) {
      ...
      myChart.setOption({
        ...
        series: [
          ...
          {
            name: data.yA[1],
            type: data.series&&data.series[1].type||"line",
            label: {
              show: true,
              color: "#666",
              formatter: "{c}亿"
